...
var fluidPlayerClass = {
    ...
    getIconClickThroughFromLinear: function(linear) {
        ...
        if (iconClickThrough.length) {
            ...
            this.displayOptions.vastOptions.adCTAText = !1
        }
        return ''
    },
        ...
        for (var i = 0; i < trackingEvents.length; i++) {
            ...
            switch (eventType) {
                ...
                case 'complete':
                    ...
                    tmpOptions.stopTracking[eventType] = !1;
                    break;
                case 'progress':
                    if (typeof tmpOptions.tracking[eventType] === 'undefined') {
                        tmpOptions.tracking[eventType] = []
    ...
    prepareVast: function(roll) {
        ...
        for (var i = 0; i < list.length; i++) {
            var adListId = list[i];
            if (player.adList[adListId].vastLoaded !== !0 && player.adList[adListId].error !== !0) {
        ...
        if (typeof creative !== 'undefined' && creative.length) {
            ...
            if (!player.hasVastAdTagUri(xmlResponse) && player.hasInLine(xmlResponse)) {
                ...
                if (typeof tmpOptions.mediaFile !== 'undefined' || typeof tmpOptions.staticResource !== 'undefined') {
                    player.adList[adListId].vastLoaded = !0;
                    player.adPool[adListId] = Object.assign({}, tmpOptions);
                    var event = document.createEvent('Event');
                    event.initEvent('adId_' + adListId, !1, !0);
    ...
    resolveVastTag: function(vastTag, numberOfRedirects, adListId, tmpOptions) {
        ...
        if (numberOfRedirects <= player.displayOptions.vastOptions.maxAllowedVastTagRedirects) {
            player.sendRequest(vastTag, !0, player.displayOptions.vastOptions.vastTimeout, handleXmlHttpReq)
        }
        numberOfRedirects++
    },
    stopProcessAndReportError: function(adListId) {
        var player = this;
        player.adList[adListId].error = !0;
    ...
    playRoll: function(adListId) {
        ...
        var playVideoPlayer = function(adListId) {
            player.switchPlayerToVastMode = function() {
                ...
                player.toggleLoader(!1);
                player.adList[adListId].played = !0;
                player.adFinished = !1;
                videoPlayerTag.play();
    ...
    trackSingleEvent: function(eventType, eventSubType) {
        ...
        switch (eventType) {
            ...
            case 'thirdQuartile':
            case 'complete':
                if (player.vastOptions.stopTracking[eventType] === !1) {
            ...
            case 'progress':
                player.vastOptions.tracking.progress[eventSubType].elements.forEach(function(currentValue, index) {
                    if (player.vastOptions.tracking.progress[eventSubType].stopTracking === !1 && player.vastOptions.tracking.progress[eventSubType].elements.length) {
                        trackingUris = player.vastOptions.tracking.progress[eventSubType].elements
                    }
                    player.vastOptions.tracking.progress[eventSubType].stopTracking = !0
                });
                break;
            case 'impression':
    ...
    createNonLinearStatic: function(adListId) {
        var player = this;
        var videoPlayerTag = document.getElementById(player.videoPlayerId);
        if (!player.adPool.hasOwnProperty(adListId) || player.adPool[adListId].error === !0) {
            player.announceLocalError(101);
            return
        }
        player.vastOptions = player.adPool[adListId];
    ...
    preRoll: function(event) {
        ...
        player.firstPlayLaunched = !0;
        var adListId = event.type.replace('adId_', '');
        if (player.adList[adListId].played === !0) {
            return
        }
        if (player.adList[adListId].adType == 'linear') {
            player.toggleLoader(!0);
    ...
    showAdMarkers: function() {
        ...
        for (var i = 0; i < adMarkers.length; ++i) {
            ...
            if (player.adList[adListId].played === !1) {
                item.style.display = ''
            }
        }
    },
    ...
    hasValidOnPauseAd: function() {
        ...
        return onPauseAd.length != 0 && player.adList[onPauseAd[0]] && player.adList[onPauseAd[0]].error === !1
    },
    toggleOnPauseAd: function() {
        ...
        if (player.hasValidOnPauseAd() && !player.isCurrentlyPlayingAd) {
            ...
            if (onPauseAd && videoPlayerTag.paused) {
                ...
                player.adList[adListId].played = !1;
        ...
        player.timer = setInterval(function() {
            for (var keyTime in player.timerPool) {
                ...
                if (player.timerPool[keyTime] && player.timerPool[keyTime].hasOwnProperty('playRoll')) {
                    ...
                    if (player.adList[adIdToCheck].played == !1) {
    ...
    onMainVideoEnded: function() {
        ...
        if (player.displayOptions.layoutControls.loop === !0) {
    ...
    addSkipButton: function() {
        ...
        videoPlayerTag.addEventListener('timeupdate', this.decreaseSkipOffset, !1)
    },
    addAdCountdown: function() {
        ...
        videoPlayerTag.addEventListener('timeupdate', this.decreaseAdCountdown, !1);
    ...
    decreaseSkipOffset: function decreaseSkipOffset() {
        ...
        if (btn) {
            if (sec >= 1) {
                ...
                btn.innerHTML = '<a href="javascript:;" id="skipHref_' + player.videoPlayerId + '" onclick="fluidPlayerClass.getInstanceById(\'' + player.videoPlayerId + '\').pressSkipButton();">' + player.displayOptions.vastOptions.skipButtonClickCaption + '</a>';
    ...
    addClickthroughLayer: function() {
        ...
        var isIos9orLower = player.mobileInfo.device === 'iPhone' && player.mobileInfo.userOsMajor !== !1 && player.mobileInfo.userOsMajor <= 9;
        clickthroughLayer.onclick = function() {
            if (videoPlayerTag.paused) {
                if (isIos9orLower && !player.suppressClickthrough) {
                    openClickthrough();
                    player.suppressClickthrough = !0
                } else {
                    videoPlayerTag.play()
                }
            } else {
    ...
    onRecentWaiting: function() {
        var player = fluidPlayerClass.getInstanceById(this.id);
        player.recentWaiting = !0;
        setTimeout(function() {
            player.recentWaiting = !1
        }, 1000)
    },
    onFluidPlayerPause: function() {
        ...
        setTimeout(function() {
            ...
            if (!player.recentWaiting) {
                ...
                event.initEvent('fluidplayerpause', !1, !0);
    ...
    contolDurationUpdate: function(videoPlayerId) {
        var player = fluidPlayerClass.getInstanceById(videoPlayerId);
        var videoPlayerTag = document.getElementById(videoPlayerId);
        var durationText = player.pad(parseInt(videoPlayerTag.currentTime / 60)) + ':' + player.pad(parseInt(videoPlayerTag.currentTime % 60)) + ' / ' + player.pad(parseInt(player.currentVideoDuration / 60)) + ':' + player.pad(parseInt(player.currentVideoDuration % 60));
    ...
    contolVolumebarUpdate: function(videoPlayerId) {
        ...
        if (videoPlayerTag.volume) {
            ...
            player.fluidStorage.fluidMute = !1
        }
    ...
    onProgressbarMouseDown: function(videoPlayerId, event) {
        var player = fluidPlayerClass.getInstanceById(videoPlayerId);
        player.displayOptions.layoutControls.playPauseAnimation = !1;
    ...
    handleMouseenterForKeyboard: function() {
        ...
        document.addEventListener('keydown', videoPlayerInstance.captureKey, !0)
    },
    keyboardControl: function() {
        var player = this;
        var videoPlayer = document.getElementById('fluid_video_wrapper_' + player.videoPlayerId);
        videoPlayer.addEventListener('click', player.handleMouseenterForKeyboard, !1);
        window.addEventListener('click', function(e) {
            ...
            if (document.getElementById('fluid_video_wrapper_' + videoInstanceId).contains(e.target) || e.target.id == 'skipHref_' + videoInstanceId) {} else {
                document.removeEventListener('keydown', videoPlayerInstance.captureKey, !0);
        ...
        if (!isFirstStart || preRolls.length == 0) {
            ...
            if (player.displayOptions.layoutControls.layout !== 'browser') {
                if (videoPlayerTag.paused) {
                    ...
                    this.playPauseAnimationToggle(!0)
                } else if (!isFirstStart) {
                    videoPlayerTag.pause();
                    this.playPauseAnimationToggle(!1)
                }
            }
            ...
            player.isCurrentlyPlayingAd = !0;
            if (browserVersion.browserName == 'Safari' || (player.mobileInfo.userOs !== !1 && player.mobileInfo.userOs == 'Android' && browserVersion.browserName == 'Google Chrome')) {
    ...
    setCustomContextMenu: function() {
        ...
        divContextMenu.innerHTML = '<ul>' + '    <li id="' + player.videoPlayerId + 'context_option_play">Play</li>' + '    <li id="' + player.videoPlayerId + 'context_option_mute">Mute</li>' + '    <li id="' + player.videoPlayerId + 'context_option_fullscreen">Fullscreen</li>' + '    <li id="' + player.videoPlayerId + 'context_option_homepage">Fluid Player ' + player.version + '</li>' + '</ul>';
    ...
    setLayout: function() {
        ...
        switch (this.displayOptions.layoutControls.layout) {
            case 'browser':
                break;
            default:
                videoPlayerTag.addEventListener('play', player.initialPlay, !1);
    ...
    setupThumbnailPreviewVtt: function() {
        var player = this;
        player.sendRequest(player.displayOptions.layoutControls.timelinePreview.file, !0, player.displayOptions.vastOptions.vastTimeout, function() {
                ...
                for (var i = 0; i < vttRawData.cues.length; i++) {
                    ...
                    if (tempThumbnailData.length === 2 && tempThumbnailData[1].indexOf('xywh=') === 0) {
                        ...
                        if (tempThumbnailCoordinates.length === 4) {
                            player.displayOptions.layoutControls.timelinePreview.spriteImage = !0;
        ...
        if (player.displayOptions.layoutControls.timelinePreview && typeof player.displayOptions.layoutControls.timelinePreview.file === 'string' && typeof player.displayOptions.layoutControls.timelinePreview.type === 'string') {
            switch (player.displayOptions.layoutControls.timelinePreview.type) {
                case 'VTT':
                    ...
                    document.getElementById(player.videoPlayerId + '_fluid_controls_progress_container').addEventListener(eventOn, player.drawTimelinePreview.bind(player), !1);
    ...
    setCurrentTimeAndPlay: function(newCurrentTime, shouldPlay) {
        ...
        videoPlayerTag.addEventListener('loadedmetadata', loadedMetadata, !1);
        videoPlayerTag.load()
    },
    initLogo: function() {
        var player = this;
    ...
    mainVideoReady: function() {
        var videoPlayerTag = this;
        var player = fluidPlayerClass.getInstanceById(this.id);
        if (player.mainVideoDuration == 0 && !player.isCurrentlyPlayingAd && player.mainVideoReadyState === !1) {
            player.mainVideoDuration = videoPlayerTag.duration;
            player.mainVideoReadyState = !0;
    ...
    hasControlBar: function() {
        return document.getElementById(this.videoPlayerId + '_fluid_controls_container') && this.displayOptions.layoutControls.layout != 'browser' ? !0 : !1
    },
    isControlBarVisible: function() {
        var player = this;
        if (player.hasControlBar() === !1) {
            return !1
        }
    ...
    hideControlBar: function() {
        ...
        if (videoPlayerInstance.isCurrentlyPlayingAd && !videoPlayerTag.paused && videoPlayerInstance.displayOptions.layoutControls.layout !== 'browser') {
            videoPlayerInstance.toggleAdCountdown(!0)
        }
    ...
    showControlBar: function() {
        ...
        var videoPlayerTag = document.getElementById(videoInstanceId);
        if (videoPlayerInstance.isCurrentlyPlayingAd && !videoPlayerTag.paused && videoPlayerInstance.displayOptions.layoutControls.layout !== 'browser') {
            videoPlayerInstance.toggleAdCountdown(!1)
        }
        if (videoPlayerInstance.hasControlBar()) {
            var divVastControls = document.getElementById(videoPlayerInstance.videoPlayerId + '_fluid_controls_container');
            var fpLogo = document.getElementById(videoPlayerInstance.videoPlayerId + '_logo');
    ...
    initMute: function() {
        var player = this;
        if (player.displayOptions.layoutControls.mute === !0) {
            var videoPlayerTag = document.getElementById(player.videoPlayerId);
            videoPlayerTag.volume = 0
        }
    },
    initLoop: function() {
        ...
        if (player.displayOptions.layoutControls.loop !== null) {
            ...
            player.displayOptions.layoutControls.loop = !0
        }
    },
    ...
    theatreToggle: function() {
        ...
        var event = document.createEvent('CustomEvent');
        event.initEvent(theatreEvent, !1, !0);
    ...
    initialiseDash: function() {
        if (typeof(window.MediaSource || window.WebKitMediaSource) === 'function') {
            ...
            dashPlayer.getDebug().setLogToBrowserConsole(!1);
    ...
    init: function(idVideoPlayer, options) {
        ...
        player.firstPlayLaunched = !1;
        player.suppressClickthrough = !1;
        ...
        player.showTimeOnHover = !0;
        player.initialAnimationSet = !0;
        player.theatreMode = !1;
        player.theatreModeAdvanced = !1;
        player.fullscreenMode = !1;
        player.originalWidth = videoPlayer.offsetWidth;
        player.originalHeight = videoPlayer.offsetHeight;
        player.dashPlayer = !1;
        player.hlsPlayer = !1;
        player.dashScriptLoaded = !1;
        player.hlsScriptLoaded = !1;
        player.isPlayingMedia = !1;
        player.isSwitchingSource = !1;
        player.isLoading = !1;
        player.isInIframe = player.inIframe();
        player.mainVideoReadyState = !1;
        ...
        player.initialiseStreamers();
        videoPlayer.addEventListener('webkitfullscreenchange', player.recalculateAdDimensions, !1);
        videoPlayer.addEventListener('fullscreenchange', player.recalculateAdDimensions, !1);
        videoPlayer.addEventListener('waiting', player.onRecentWaiting, !1);
        videoPlayer.addEventListener('pause', player.onFluidPlayerPause, !1);
        videoPlayer.addEventListener('loadedmetadata', player.mainVideoReady, !1);
        videoPlayer.addEventListener('durationchange', function() {
            player.currentVideoDuration = player.getCurrentVideoDuration()
        }, !1);
        videoPlayer.addEventListener('error', player.onErrorDetection, !1);
        videoPlayer.addEventListener('ended', player.onMainVideoEnded, !1);
        player.currentVideoDuration = player.getCurrentVideoDuration();
        ...
        if (!player.mobileInfo.userOs) {
            videoWrapper.addEventListener('mouseleave', player.handleMouseleave, !1);
            videoWrapper.addEventListener('mouseenter', player.showControlBar, !1)
        } else {
    ...
    toggleLogo: function(logo) {
        ...
        this.displayOptions.layoutControls.logo.hideWithControls = logo.hideWithControls ? logo.hideWithControls : !1;
        this.displayOptions.layoutControls.logo.showOverAds = logo.showOverAds ? logo.showOverAds : !1;
        if (logoBlock) {
            logoBlock.remove()
        }
        this.initLogo()
